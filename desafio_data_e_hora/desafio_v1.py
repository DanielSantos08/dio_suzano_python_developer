...
class Cliente:
    ...
    def realizar_transacao(self, conta, transacao):
        # Validar o número de transações diárias (limite de 10)
        transacoes_hoje = conta.historico.transacoes_do_dia()

        if len(transacoes_hoje) >= 10:
            print("\n@@@ Você excedeu o número de transações permitidas para hoje! @@@")
            print(f"Limite diário: 10 transações | Realizadas hoje: {len(transacoes_hoje)}")
            return
...
class ContaCorrente(Conta):
    ...
    def sacar(self, valor):
        # Contar apenas os saques realizados hoje
...
class Historico:
    ...
    def transacoes_do_dia(self):
        """Retorna todas as transações realizadas no dia atual"""
        data_atual = date.today().strftime("%d-%m-%Y")
        transacoes_hoje = []

        for transacao in self._transacoes:
            # Extrair apenas a data da string "dd-mm-yyyy HH:MM:SS"
...
def exibir_extrato(clientes):
    ...
    # Mostrar informações sobre transações do dia
    transacoes_hoje = conta.historico.transacoes_do_dia()
    print(f"Transações realizadas hoje: {len(transacoes_hoje)}/10")
    print("==========================================")
